...
plot_i = ny_noaa %>%
  ...
  pivot_longer(tmax_c:tmin_c, names_to = "tmax_vs_tmin", values_to = "temp") %>%
  ggplot(aes(x = year, y = temp, color = tmax_vs_tmin)) +
  geom_line() + 
  labs(title = "maximum vs minimum temperature over years",
       x = "year", y = "temperature (C)")

plot_ii = ny_noaa %>%
  filter(snow > 0, snow < 100) %>%
  ggplot(aes(x = year, y = snow, color = )) +
  geom_line()
